  ...
  const sendMessage = async (manualText?: string) => {
    ...
    try {
      ...
      if (data.content) {
        ...
        // A. Extract JSON Data for Dashboard Sync
        if (fullResponse.includes("###DATA###")) {
          try {
            ...
            // Remove metadata from text before showing in bubbles
        ...
        // B. Protect Image URLs from splitting
      ...
      const textParts = placeholderText.split(/[.?!]+/).filter((p: string) => p.trim().length > 0);
        ...
        for (let part of textParts) {
          ...
          // Restore Image Link if placeholder found
          if (finalMsg.includes("[[IMAGE_FIX]]") && foundImages.length > 0) {
            finalMsg = foundImages.shift() || "";
          }
    ...
    <div className="flex flex-col h-[100dvh] relative overflow-hidden text-white font-sans bg-[#0d0216]">
            ...
            return (
              <motion.div key={m.id} initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} className={`flex flex-col w-full ${isUser ? 'items-end' : 'items-start'} group`}>
                ...
                <div className="relative flex items-center gap-3 group max-w-[85%]">
                  <div onDoubleClick={() => setReplyingTo(m)} className={`relative transition-all duration-300 ${emojiOnly ? 'text-6xl py-2 drop-shadow-[0_10px_20px_rgba(225,29,72,0.3)]' : `px-4 py-3 rounded-[24px] text-[15px] shadow-2xl border ${isUser ? 'bg-gradient-to-br from-rose-600/90 to-pink-700/90 border-white/10 text-white rounded-br-none' : 'bg-white/5 backdrop-blur-2xl border-white/10 text-rose-50 rounded-bl-none'}`}`}>
                    {m.content}
                    {m.reaction && (
      ...
      <footer className="relative z-20 p-5 bg-black/40 backdrop-blur-3xl border-t border-white/5">
        ...
        <div className={`max-w-2xl mx-auto flex items-center gap-3 bg-white/5 border border-white/10 p-2 px-6 shadow-2xl transition-all ${replyingTo ? 'rounded-b-2xl' : 'rounded-full'} focus-within:border-rose-500/40`}>
          <input className="flex-1 bg-transparent outline-none py-3 text-[15px] placeholder-white/20" placeholder="Talk to Maya..." value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && sendMessage()} />
